/**
 * E2E — Full system verification (realistic business day)
 * Run: npx ts-node --compiler-options {"module":"CommonJS"} scripts/test-full-system.ts
 */
...
function assertStep(
  ...
  if (ok) console.log(`✅ PASS [${step}] ${check} — ${actual}`);
}
...
async function getCustomerBalance(
  ...
  return row ? new Prisma.Decimal(row.balance) : null;
}
...
async function sumJournalForEvent(
  prisma: PrismaService,
  eventId: string,
...
async function computeArBalanceFromLedger(
  prisma: PrismaService,
  customerId: string,
): Promise<Prisma.Decimal> {
  const ar = await prisma.account.findUnique({ where: { code: '1200' } });
  if (!ar) return new Prisma.Decimal(0);
...
async function computeApBalanceFromLedger(
  prisma: PrismaService,
  supplierId: string,
): Promise<Prisma.Decimal> {
  const ap = await prisma.account.findUnique({ where: { code: '2000' } });
  if (!ap) return new Prisma.Decimal(0);
  const lines = await prisma.journalLine.findMany({
...
async function computeStockFromMovements(
  prisma: PrismaService,
  productId: string,
  locationId: string,
): Promise<Prisma.Decimal> {
...
function printResultsTable(): void {
  ...
  for (const r of stepResults) {
    console.log(
      `| ${r.step} | ${r.check} | ${r.expected} | ${r.actual} | ${r.status} |`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  const ids: {
    ...
    eventLogIds: string[];
  } = { saleIds: [], eventClientUuids: [], eventLogIds: [] };
  ...
  try {
    ...
    if (receiveEvent) ids.eventLogIds.push(receiveEvent.id);
    ...
    assertStep(
      ...
      'SupplierBalance',
      '2000',
      supBalAfterPurchase?.toString() ?? 'null',
      supBalAfterPurchase?.equals(2000) ?? false,
    );
    ...
    const cashSaleDomain = cashSaleResult.result as { domain?: { saleId: string }; eventId: string };
    ...
    if (!cashSale) throw new Error('Cash sale row missing');
    ...
    let cashDr = new Prisma.Decimal(0);
    let salesCr = new Prisma.Decimal(0);
    let cogsDr = new Prisma.Decimal(0);
    let invCr = new Prisma.Decimal(0);
    for (const entry of cashEntries) {
      for (const line of entry.lines) {
        if (line.account.code === '1000') cashDr = cashDr.plus(line.debit);
        if (line.account.code === '4000') salesCr = salesCr.plus(line.credit);
        if (line.account.code === '5000') cogsDr = cogsDr.plus(line.debit);
        if (line.account.code === '1100') invCr = invCr.plus(line.credit);
      }
    }
    ...
    assertStep(
      ...
      'Journal balanced',
      'debit=credit',
      `${cashJournal.debit.toString()}=${cashJournal.credit.toString()}`,
      cashJournal.debit.equals(cashJournal.credit),
    );
    ...
    const debtSaleDomain = debtSaleResult.result as { domain?: { saleId: string }; eventId: string };
    ...
    if (debtSale) ids.eventClientUuids.push(debtSale.clientUuid);
    ...
    assertStep(
      ...
      'Customer2 balance',
      '700',
      cust2BalAfterDebt?.toString() ?? 'null',
      cust2BalAfterDebt?.equals(700) ?? false,
    );

    // --- Step 4: Return ---
    console.log('\n--- Step 4: Return (customer2, product2 qty=5, credit) ---');
    const returnResult = await salesService.createReturn(
      debtSaleDomain.domain!.saleId,
      {
        items: [
          {
            productId: product2.id,
    ...
    const returnDomain = returnResult.result as { eventId: string };
    ...
    assertStep(
      ...
      'Customer2 balance',
      '525',
      cust2BalAfterReturn?.toString() ?? 'null',
      cust2BalAfterReturn?.equals(525) ?? false,
    );
    ...
    const custPayDomain = custPayResult.result as {
    ...
    if (custPayment) ids.eventClientUuids.push(custPayment.clientUuid);
    ...
    assertStep(
      ...
      'Customer2 balance',
      '225',
      cust2BalAfterPay?.toString() ?? 'null',
      cust2BalAfterPay?.equals(225) ?? false,
    );
    ...
    const supPayDomain = supPayResult.result as {
      domain?: { paymentId: string };
      eventId: string;
    };
    ...
    if (supPayment) ids.eventClientUuids.push(supPayment.clientUuid);
    ...
    assertStep(
      ...
      'SupplierBalance',
      '1000',
      supBalAfterPay?.toString() ?? 'null',
      supBalAfterPay?.equals(1000) ?? false,
    );

    // --- Step 7: Stock reconcile ---
    ...
    const reconcileDomain = reconcileResult.result as { eventId: string };
    ...
    let shrinkageDebit = new Prisma.Decimal(0);
    ...
    assertStep(
      ...
      'Product1 stock',
      '68',
      stock1AfterReconcile.toString(),
      stock1AfterReconcile.equals(68),
    );
    assertStep(
      '7',
      'Dr InventoryShrinkage',
      '20',
      shrinkageDebit.toString(),
      shrinkageDebit.equals(20),
    );
    ...
    const stock1Movements = await computeStockFromMovements(
      prisma,
      product1.id,
      location.id,
    );
    assertStep(
      'B',
      'Product1 view=movements',
      stock1Movements.toString(),
      stock1View.toString(),
      stock1View.equals(stock1Movements),
    );

    const stock2View = await getStockQty(prisma, product2.id, location.id);
    const stock2Movements = await computeStockFromMovements(
      prisma,
      product2.id,
      location.id,
    );
    assertStep(
      'B',
      'Product2 view=movements',
      stock2Movements.toString(),
      stock2View.toString(),
      stock2View.equals(stock2Movements),
    );
    ...
    for (const [label, customerId] of [
      ...
      const viewVal = viewBal ?? new Prisma.Decimal(0);
      assertStep(
        'C',
        `${label} AR view=ledger`,
        ledgerBal.toString(),
        viewVal.toString(),
        viewVal.equals(ledgerBal),
      );
    }
    ...
    assertStep(
      'D',
      'Supplier AP view=ledger',
      supLedger.toString(),
      supView?.toString() ?? 'null',
      supView?.equals(supLedger) ?? false,
    );

    // E) All EventLog APPLIED
    let allApplied = true;
    ...
    // F) Idempotency — replay cash sale
    ...
    assertStep(
      ...
      'Idempotent replay',
      'no duplicates',
      `mov ${movementCountBefore}->${movementCountAfter}, je ${journalCountBefore}->${journalCountAfter}, sales ${saleCountBefore}->${saleCountAfter}`,
      idempotentOk,
    );
...
async function cleanup(
  ...
  const customerIds = [ids.customer1Id, ids.customer2Id].filter(Boolean) as string[];
  ...
  await prisma.payment.deleteMany({
    where: {
      ...
      ].filter(Boolean) as { partyId: string | { in: string[] } }[],
    },
  });
  ...
  const productIds = [ids.product1Id, ids.product2Id].filter(Boolean) as string[];
