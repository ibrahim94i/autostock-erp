...
async function main(): Promise<void> {
  ...
  try {
    ...
    assertTrue(
      po.items.length === 1 &&
        new Prisma.Decimal(po.items[0].qty).equals(20) &&
        new Prisma.Decimal(po.items[0].unitCost).equals(15),
      'PO item qty=20 unitCost=15',
    );

    console.log('\nStep 3: Receive PO (PURCHASE_RECEIVED)...');
    const dispatchResult = await purchasingService.receive(
      po.id,
    ...
    assertTrue(
      ...
      dispatchResult.status === 'REJECTED'
        ? (dispatchResult as { reason: string }).reason
        : undefined,
    );
    ...
    if (!eventLog) fail('EventLog exists for clientUuid');
    ...
    if (!movement) fail('StockMovement created');
    assertTrue(movement.direction === 'IN', 'StockMovement direction IN', movement.direction);
    assertTrue(
      new Prisma.Decimal(movement.quantity).equals(20),
    ...
    if (!stockBalance) fail('StockBalanceView exists');
    assertTrue(
      new Prisma.Decimal(stockBalance.quantity).equals(20),
      'StockBalanceView quantity = 20',
      stockBalance.quantity.toString(),
    );
    ...
    for (const line of entry.lines) {
      ...
      if (line.account.code === '1100') inventoryDebit = inventoryDebit.plus(line.debit);
      if (line.account.code === '2000') apCredit = apCredit.plus(line.credit);
    }
    ...
    assertTrue(
      inventoryDebit.equals(300),
      'Dr Inventory = 300',
      inventoryDebit.toString(),
    );
    assertTrue(
      apCredit.equals(300),
      'Cr AccountsPayable = 300',
      apCredit.toString(),
    );
    ...
    if (!supplierBalance) fail('SupplierBalanceView exists');
    assertTrue(
      new Prisma.Decimal(supplierBalance.balance).equals(300),
      'SupplierBalanceView balance = 300',
      supplierBalance.balance.toString(),
    );

    const poAfter = await prisma.purchaseOrder.findUnique({ where: { id: po.id } });
    if (!poAfter) fail('PO still exists');
    ...
    try {
      ...
      if (error instanceof BadRequestException) {
        rejected = true;
        pass(
          'Second receive rejected',
          error.message ?? 'already received or invalid',
        );
      } else {
        throw error;
      }
    ...
    assertTrue(
      ...
      `${movementCountBefore} -> ${movementCountAfter}`,
    );
    assertTrue(
