...
async function sumJournalForEvent(
  ...
  salesCredit: Prisma.Decimal;
  cogsDebit: Prisma.Decimal;
  inventoryCredit: Prisma.Decimal;
  ...
  let salesCredit = new Prisma.Decimal(0);
  let cogsDebit = new Prisma.Decimal(0);
  let inventoryCredit = new Prisma.Decimal(0);
  ...
  for (const entry of entries) {
    for (const line of entry.lines) {
      ...
      if (line.account.code === '1000') cashDebit = cashDebit.plus(line.debit);
      if (line.account.code === '4000') salesCredit = salesCredit.plus(line.credit);
      if (line.account.code === '5000') cogsDebit = cogsDebit.plus(line.debit);
      if (line.account.code === '1100') inventoryCredit = inventoryCredit.plus(line.credit);
      if (line.account.code === '1200') arDebit = arDebit.plus(line.debit);
    }
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result1 = r1.result as {
      domain?: { saleId: string };
      eventId: string;
    };
    const sale1 = await prisma.sale.findUnique({
      where: { id: result1.domain!.saleId },
    });
    if (!sale1) fail('Test 1 Sale row exists');
    ...
    if (!event1) fail('Test 1 EventLog exists');
    assertTrue(event1.status === 'APPLIED', 'Test 1 EventLog APPLIED');
    assertTrue(sale1.status === 'completed', 'Test 1 Sale status completed');

    assertTrue(
      (await getStockQty(prisma, product.id, location.id)).equals(90),
      'Test 1 StockBalanceView = 90',
    );
    ...
    assertTrue(
      j1.inventoryCredit.equals(100),
      'Test 1 Cr Inventory = 100',
      j1.inventoryCredit.toString(),
    );
    ...
    if (r2.status !== 'APPLIED') fail('Test 2 dispatch APPLIED');

    const result2 = r2.result as { domain?: { saleId: string } };
    const sale2 = await prisma.sale.findUnique({
      where: { id: result2.domain!.saleId },
    });
    if (!sale2) fail('Test 2 Sale row exists');
    ...
    if (!event2) fail('Test 2 EventLog exists');

    assertTrue(
      (await getStockQty(prisma, product.id, location.id)).equals(70),
      'Test 2 StockBalanceView = 70',
    );
    ...
    if (!custBal) fail('Test 2 CustomerBalanceView exists');
    assertTrue(
      new Prisma.Decimal(custBal.balance).equals(360),
      'Test 2 CustomerBalanceView = 360',
      custBal.balance.toString(),
    );

    // --- Test 3: Rejected insufficient stock ---
    ...
    if (r3.status === 'REJECTED') {
      assertTrue(
        r3.reason.toLowerCase().includes('insufficient stock'),
        'Test 3 rejection reason mentions insufficient stock',
        r3.reason,
      );
    }
    ...
    // --- Test 4: Atomicity — onCommit failure rolls back ledger ---
    ...
    assertTrue(
      ...
      'Test 4 StockBalanceView unchanged after rollback',
    ...
    assertTrue(
      ...
      'Test 4 no new StockMovement after rollback',
    ...
    assertTrue(
      ...
      'Test 4 no new JournalEntry after rollback',
    ...
    assertTrue(
      saleCountAfterT5 === saleCountBeforeT5,
      'Test 5 no duplicate Sale row',
      `${saleCountBeforeT5} -> ${saleCountAfterT5}`,
    );
