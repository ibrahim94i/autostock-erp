...
async function sumReturnJournalForEvent(prisma, eventId) {
    ...
    let salesReturnsDebit = new client_1.Prisma.Decimal(0);
    let cashCredit = new client_1.Prisma.Decimal(0);
    let arCredit = new client_1.Prisma.Decimal(0);
...
async function main() {
    ...
    try {
        ...
        if (saleResult.status !== 'APPLIED')
            fail('Setup debt sale not APPLIED');
        const saleDomain = saleResult.result;
        const saleId = saleDomain.domain.saleId;
        ids.saleIds.push(saleId);
        const sale = await prisma.sale.findUnique({ where: { id: saleId } });
        if (!sale)
            fail('Setup Sale row exists');
        ids.eventClientUuids.push(sale.clientUuid);
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(70), 'Stock after debt sale = 70');
        const custBalAfterSale = await prisma.customerBalanceView.findUnique({
            where: { customerId: customer.id },
        });
        if (!custBalAfterSale)
            fail('CustomerBalanceView exists after sale');
        assertTrue(new client_1.Prisma.Decimal(custBalAfterSale.balance).equals(540), 'CustomerBalanceView = 540 after debt sale', custBalAfterSale.balance.toString());
        console.log('\nTest 1: Cash refund (qty=5, refundAmount=90)...');
        const r1 = await salesService.createReturn(saleId, {
        ...
        const t1Domain = r1.result;
        assertTrue((t1Domain.domain?.returnIds?.length ?? 0) >= 1, 'Test 1 Return row created', JSON.stringify(t1Domain.domain?.returnIds));
        ...
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(75), 'Test 1 StockBalanceView = 75 (70 + 5)');
        const t1Movement = await prisma.stockMovement.findFirst({
            where: { eventId: t1Event.id },
        });
        if (!t1Movement)
            fail('Test 1 StockMovement exists');
        assertTrue(t1Movement.direction === 'IN', 'Test 1 StockMovement direction IN');
        assertTrue(new client_1.Prisma.Decimal(t1Movement.quantity).equals(5), 'Test 1 StockMovement qty=5', t1Movement.quantity.toString());
        assertTrue(t1Movement.sourceType === 'RETURN', 'Test 1 StockMovement sourceType RETURN', t1Movement.sourceType);
        ...
        assertTrue(j1.salesReturnsDebit.equals(90), 'Test 1 Dr SalesReturns = 90', j1.salesReturnsDebit.toString());
        assertTrue(j1.cashCredit.equals(90), 'Test 1 Cr Cash = 90', j1.cashCredit.toString());
        assertTrue(j1.inventoryDebit.equals(50), 'Test 1 Dr Inventory = 50', j1.inventoryDebit.toString());
        assertTrue(j1.cogsCredit.equals(50), 'Test 1 Cr COGS = 50', j1.cogsCredit.toString());
        const test1ReturnClientUuid = t1Event.clientUuid;
        console.log('\nTest 2: Credit refund (qty=5, refundAmount=90)...');
        const r2 = await salesService.createReturn(saleId, {
            items: [{ ...saleItem, qty: 5 }],
            refundMethod: 'credit',
            refundAmount: 90,
        ...
        if (r2.status !== 'APPLIED')
            fail('Test 2 not APPLIED');
        const t2Domain = r2.result;
        ...
        assertTrue((t2Domain.domain?.returnIds?.length ?? 0) >= 1, 'Test 2 Return row created');
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(80), 'Test 2 StockBalanceView = 80 (75 + 5)');
        ...
        assertTrue(j2.salesReturnsDebit.equals(90), 'Test 2 Dr SalesReturns = 90', j2.salesReturnsDebit.toString());
        assertTrue(j2.arCredit.equals(90), 'Test 2 Cr AccountsReceivable = 90', j2.arCredit.toString());
        assertTrue(j2.inventoryDebit.equals(50), 'Test 2 Dr Inventory = 50', j2.inventoryDebit.toString());
        assertTrue(j2.cogsCredit.equals(50), 'Test 2 Cr COGS = 50', j2.cogsCredit.toString());
        const custBalAfterT2 = await prisma.customerBalanceView.findUnique({
            where: { customerId: customer.id },
        });
        if (!custBalAfterT2)
            fail('Test 2 CustomerBalanceView exists');
        assertTrue(new client_1.Prisma.Decimal(custBalAfterT2.balance).equals(450), 'Test 2 CustomerBalanceView = 450 (540 - 90)', custBalAfterT2.balance.toString());
        ...
        let t3Message = '';
        try {
            await salesService.createReturn(saleId, {
        ...
        assertTrue(t3Message.toLowerCase().includes('exceeds') ||
            t3Message.toLowerCase().includes('sold quantity'), 'Test 3 rejection mentions exceeds sold quantity', t3Message);
        ...
        assertTrue(returnCountAfterT3 === returnCountBeforeT3, 'Test 3 no new Return row', `${returnCountBeforeT3} -> ${returnCountAfterT3}`);
        ...
        assertTrue(journalCountAfterT3 === journalCountBeforeT3, 'Test 3 no new JournalEntry', `${journalCountBeforeT3} -> ${journalCountAfterT3}`);
        ...
        if (drainResult.status !== 'APPLIED')
            fail('Test 4 drain sale not APPLIED');
        const drainDomain = drainResult.result;
        ids.saleIds.push(drainDomain.domain.saleId);
        const drainSale = await prisma.sale.findUnique({
            where: { id: drainDomain.domain.saleId },
        });
        if (drainSale)
            ids.eventClientUuids.push(drainSale.clientUuid);
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(0), 'Test 4 stock drained to 0');
        const r4 = await salesService.createReturn(saleId, {
        ...
        if (r4.status !== 'APPLIED')
            fail('Test 4 return not APPLIED');
        const t4Domain = r4.result;
        const t4Event = await prisma.eventLog.findUnique({
            where: { id: t4Domain.eventId },
        });
        if (t4Event)
            ids.eventClientUuids.push(t4Event.clientUuid);
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(2), 'Test 4 StockBalanceView = 2 after return IN');
        ...
        assertTrue(returnCountAfterT5 === returnCountBeforeT5, 'Test 5 no duplicate Return row', `${returnCountBeforeT5} -> ${returnCountAfterT5}`);
