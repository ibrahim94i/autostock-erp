...
const sync_service_1 = require("../src/sync/sync.service");
...
async function getMaxServerSeq(prisma) {
...
async function main() {
    ...
    try {
        ...
        const op1Uuid = (0, crypto_1.randomUUID)();
        const op2Uuid = (0, crypto_1.randomUUID)();
        ...
        assertTrue(push1.applied.includes(op1Uuid) && push1.applied.includes(op2Uuid), 'Test 1 both clientUuids in applied', JSON.stringify(push1.applied));
        assertTrue(push1.rejected.length === 0, 'Test 1 rejected is empty');
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(40), 'Test 1 StockBalanceView = 40 (50 - 10)');
        assertTrue(push1.serverSeq > baseline, 'Test 1 serverSeq > baseline', `${baseline} -> ${push1.serverSeq}`);
        ...
        pass('Test 2 operations applied in localSeq order', 'stock 40 proves adjust(50) ran before sale(10)');
        ...
        assertTrue(push3.applied.includes(op1Uuid) && push3.applied.includes(op2Uuid), 'Test 3 replay returns same clientUuids in applied');
        ...
        assertTrue(movementCountAfterT3 === movementCountAfterT1, 'Test 3 no duplicate StockMovement', `${movementCountAfterT1} -> ${movementCountAfterT3}`);
        const journalCountAfterT3 = await prisma.journalEntry.count();
        assertTrue(journalCountAfterT3 === journalCountAfterT1, 'Test 3 no duplicate JournalEntry', `${journalCountAfterT1} -> ${journalCountAfterT3}`);
        const eventLogCountAfterT3 = await prisma.eventLog.count({
            where: { clientUuid: { in: [op1Uuid, op2Uuid] } },
        });
        assertTrue(eventLogCountAfterT3 === eventLogCountAfterT1, 'Test 3 no duplicate EventLog rows', `${eventLogCountAfterT1} -> ${eventLogCountAfterT3}`);
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(40), 'Test 3 StockBalanceView unchanged at 40');
        ...
        assertTrue(push4.rejected[0]?.clientUuid === rejectUuid, 'Test 4 rejected clientUuid matches');
        assertTrue(push4.rejected[0]?.reason.toLowerCase().includes('insufficient stock'), 'Test 4 rejection reason mentions insufficient stock', push4.rejected[0]?.reason);
        assertTrue(!push4.applied.includes(rejectUuid), 'Test 4 rejected uuid not in applied');
        assertTrue((await getStockQty(prisma, product.id, location.id)).equals(stockBeforeT4), 'Test 4 StockBalanceView unchanged', stockBeforeT4.toString());
        const movementCountAfterT4 = await prisma.stockMovement.count({
            where: { productId: product.id },
        });
        assertTrue(movementCountAfterT4 === movementCountBeforeT4, 'Test 4 no new StockMovement', `${movementCountBeforeT4} -> ${movementCountAfterT4}`);
        const journalCountAfterT4 = await prisma.journalEntry.count();
        assertTrue(journalCountAfterT4 === journalCountBeforeT4, 'Test 4 no new JournalEntry', `${journalCountBeforeT4} -> ${journalCountAfterT4}`);
        console.log('\nTest 5: Pull delta (since=baseline)...');
        const pull1 = await syncService.pull(baseline);
        console.log('Pull result:', JSON.stringify({
        ...
        assertTrue(pulledUuids.includes(op1Uuid) && pulledUuids.includes(op2Uuid), 'Test 5 changes include STOCK_ADJUSTED and SALE_CREATED clientUuids');
        for (let i = 1; i < pull1.changes.length; i++) {
            assertTrue(pull1.changes[i].serverSeq > pull1.changes[i - 1].serverSeq, 'Test 5 changes sorted ascending by serverSeq');
        }
        for (const change of pull1.changes) {
            if (change.clientUuid === op1Uuid || change.clientUuid === op2Uuid) {
                assertTrue(change.serverSeq > 0 &&
                    !!change.eventType &&
                    change.payload !== undefined &&
                    !!change.clientUuid, 'Test 5 change has required fields', change.clientUuid);
            }
        }
        ...
        assertTrue(stockChange.serverSeq < saleChange.serverSeq, 'Test 5 STOCK_ADJUSTED serverSeq before SALE_CREATED', `${stockChange.serverSeq} < ${saleChange.serverSeq}`);
        const pull2 = await syncService.pull(pull1.serverSeq);
        console.log('Pull (no new changes):', JSON.stringify({ serverSeq: pull2.serverSeq, changeCount: pull2.changes.length }, null, 2));
        assertTrue(pull2.changes.length === 0, 'Test 5 pull after last serverSeq has empty changes');
        assertTrue(pull2.serverSeq === pull1.serverSeq, 'Test 5 serverSeq unchanged when no delta', `${pull1.serverSeq} -> ${pull2.serverSeq}`);
