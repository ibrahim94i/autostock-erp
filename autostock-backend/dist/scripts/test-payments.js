...
async function sumPaymentJournalForEvent(prisma, eventId) {
    ...
    let apDebit = new client_1.Prisma.Decimal(0);
...
async function main() {
    ...
    const ids = { saleIds: [], poIds: [], eventClientUuids: [] };
    ...
    try {
        ...
        if (saleResult.status !== 'APPLIED')
            fail('Test 1 debt sale not APPLIED');
        ...
        if (!custBalBeforePay)
            fail('Test 1 CustomerBalanceView exists after sale');
        assertTrue(new client_1.Prisma.Decimal(custBalBeforePay.balance).equals(360), 'Test 1 CustomerBalanceView = 360 before payment', custBalBeforePay.balance.toString());
        ...
        assertTrue(j1.cashDebit.equals(200), 'Test 1 Dr Cash = 200', j1.cashDebit.toString());
        assertTrue(j1.arCredit.equals(200), 'Test 1 Cr AccountsReceivable = 200', j1.arCredit.toString());
        const custBalAfterPay = await prisma.customerBalanceView.findUnique({
            where: { customerId: customer.id },
        });
        if (!custBalAfterPay)
            fail('Test 1 CustomerBalanceView after payment');
        assertTrue(new client_1.Prisma.Decimal(custBalAfterPay.balance).equals(160), 'Test 1 CustomerBalanceView = 160 (360 - 200)', custBalAfterPay.balance.toString());
        const test1PaymentClientUuid = payment1.clientUuid;
        console.log('\nTest 2: Supplier payment OUT (purchase receive + payment 100)...');
        const poUuid = (0, crypto_1.randomUUID)();
        ...
        if (!supBalBeforePay)
            fail('Test 2 SupplierBalanceView exists after receive');
        assertTrue(new client_1.Prisma.Decimal(supBalBeforePay.balance).equals(150), 'Test 2 SupplierBalanceView = 150 before payment', supBalBeforePay.balance.toString());
        ...
        assertTrue(j2.apDebit.equals(100), 'Test 2 Dr AccountsPayable = 100', j2.apDebit.toString());
        assertTrue(j2.cashCredit.equals(100), 'Test 2 Cr Cash = 100', j2.cashCredit.toString());
        const supBalAfterPay = await prisma.supplierBalanceView.findUnique({
            where: { supplierId: supplier.id },
        });
        if (!supBalAfterPay)
            fail('Test 2 SupplierBalanceView after payment');
        assertTrue(new client_1.Prisma.Decimal(supBalAfterPay.balance).equals(50), 'Test 2 SupplierBalanceView = 50 (150 - 100)', supBalAfterPay.balance.toString());
        console.log('\nTest 3: P&L report...');
        ...
        assertTrue(pl.netProfit === 160, 'Test 3 Net Profit = 160 (360 - 0 - 200)', String(pl.netProfit));
        ...
        assertTrue(journalCountAfterT4 === journalCountBeforeT4, 'Test 4 no duplicate JournalEntry', `${journalCountBeforeT4} -> ${journalCountAfterT4}`);
        const paymentCountAfterT4 = await prisma.payment.count({
            where: { partyId: customer.id },
        });
        assertTrue(paymentCountAfterT4 === paymentCountBeforeT4, 'Test 4 no duplicate Payment row', `${paymentCountBeforeT4} -> ${paymentCountAfterT4}`);
        const custBalAfterT4 = await prisma.customerBalanceView.findUnique({
            where: { customerId: customer.id },
        });
        if (!custBalAfterT4 || !custBalBeforeT4)
            fail('Test 4 CustomerBalanceView exists');
        assertTrue(new client_1.Prisma.Decimal(custBalAfterT4.balance).equals(custBalBeforeT4.balance), 'Test 4 CustomerBalanceView unchanged', `${custBalBeforeT4.balance.toString()} -> ${custBalAfterT4.balance.toString()}`);
...
//# sourceMappingURL=test-payments.js.map