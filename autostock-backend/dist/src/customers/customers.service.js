...
let CustomersService = class CustomersService {
    ...
    async remove(id) {
        ...
        if (Math.abs(balance) > 0.0001) {
            throw new common_1.UnprocessableEntityException('لا يمكن حذف عميل له رصيد');
        }
        const salesCount = await this.prisma.sale.count({ where: { customerId: id } });
        if (salesCount > 0) {
            throw new common_1.UnprocessableEntityException('لا يمكن حذف عميل له مبيعات مسجّلة');
        }
    ...
    async getBalancesBulk(ids) {
        const where = ids && ids.length > 0 ? { customerId: { in: ids } } : undefined;
